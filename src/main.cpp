...
int main() {
    ...
    while (window.isOpen()) {
                ...
                if (event.type == sf::Event::MouseButtonPressed) {
                    if (event.mouseButton.button == sf::Mouse::Middle ||
                        ...
                        if (currentTool == ui::Tool::Polyline && polylineState.active && polylineState.points.size() >= 2) {
                            ...
                            // Fix: track running absolute direction so turns are computed correctly.
                            float runningAbsAngle = 0.f;
                            for (size_t i = 0; i < pts.size(); ++i) {
                                sf::Vector2f dir = (i == 0) ? (pts[1] - pts[0])
                                                             : (pts[i] - pts[i - 1]);
        ...
        if (groupClicked && multiSel) {
            ...
            auto selCopy = scene.getSelection(); // copy raw-ptr list before any mutation
            sf::Vector2f centroid(0.f, 0.f);
            for (Figure* f : selCopy)
                centroid += f->parentOrigin + f->anchor;
            centroid /= (float)selCopy.size();

            // Fix: two-pass extraction to avoid use-after-move with removeFigure.
            // Pass 1: move owned ptrs out of the scene vector (slots become nullptr).
            std::vector<std::unique_ptr<Figure>> owned;
            owned.reserve(selCopy.size());
            for (Figure* f : selCopy) {
                for (auto& up : const_cast<std::vector<std::unique_ptr<Figure>>&>(scene.getFigures())) {
                    if (up.get() == f) { owned.push_back(std::move(up)); break; }
                }
            }
            // Pass 2: erase the now-nullptr slots and clear selection bookkeeping.
            scene.clearSelection();
            auto& figs = const_cast<std::vector<std::unique_ptr<Figure>>&>(scene.getFigures());
            figs.erase(std::remove_if(figs.begin(), figs.end(),
                [](const std::unique_ptr<Figure>& p){ return p == nullptr; }), figs.end());

            for (auto& o : owned)
                if (o) composite->addChild(std::move(o));
